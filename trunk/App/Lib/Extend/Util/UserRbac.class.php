...
class UserRbac {
    ...
    static function saveAccessList($authId=null) {
        ...
        if(C('USER_AUTH_TYPE') !=2 && !$_SESSION[C('ADMIN_AUTH_KEY')] )
            $_SESSION['_ACCESS_LIST']	=	UserRbac::getAccessList($authId);
        return ;
    }
	...
	static function getRecordAccessList($authId=null,$module='') {
        ...
        //获取权限访问列表
        $accessList = UserRbac::getModuleAccessList($authId,$module);
        return $accessList;
	}
    ...
    static function checkAccess() {
        ...
        if( C('USER_AUTH_ON') ){
        	...
        	if("" != C('REQUIRE_AUTH_GROUP')) {
                //需要认证的模块
                $_group['yes'] = explode(',',strtoupper(C('REQUIRE_AUTH_GROUP')));
            }else {
                //无需认证的模块
                $_group['no'] = explode(',',strtoupper(C('NOT_AUTH_GROUP')));
            }
            if((!empty($_group['no']) && !in_array(strtoupper(GROUP_NAME),$_group['no'])) || (!empty($_group['yes']) && in_array(strtoupper(GROUP_NAME),$_group['yes']))) {
            ...
            if(!$_SESSION[C('USER_AUTH_KEY')]) {
                if(C('GUEST_AUTH_ON')) {
                    ...
                    if(!isset($_SESSION['_ACCESS_LIST']))
                        // 保存游客权限
                        UserRbac::saveAccessList(C('GUEST_AUTH_ID'));
                }else{
                    // 禁止游客访问跳转到认证网关
        ...
        if(UserRbac::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                if(C('USER_AUTH_TYPE')==2) {
                    ...
                    //通过数据库进行访问检查
                    $accessList = UserRbac::getAccessList($_SESSION[C('USER_AUTH_KEY')]);
                }else {
